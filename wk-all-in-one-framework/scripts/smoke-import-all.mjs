...
async function main() {
  ...
  const imports = [
    ...
    ['reports/junit/cypress-tdd-api.xml', 'junit-xml', 'cypress-tdd', 'API', 'TDD'],
    ['reports/junit/cypress-bdd-api.xml', 'junit-xml', 'cypress-bdd', 'API', 'BDD'],
  ...
  for (const [file, format, tool, appType, methodology] of imports) {
    ...
    if (!fs.existsSync(filePath)) {
      throw new Error(`Missing smoke report: ${filePath}`);
    }
    console.log(`Importing ${tool} (${appType})...`);
    await orchestrator.importResult(format, filePath, {
      tool,
  ...
  console.log('Smoke import complete.');
}
